...
export class EventsController {
   ...
   constructor(
    ...
    async update(
        ...
        @Body() updates: UpdateEventDto, @CurrentUser() user : User ) {
        ...
        if(user.id != event.organizerId){
            throw new ForbiddenException(null, 'You are not allowed to perform this action');
        }
         
        await this.eventsService.updateEvent(event, updates);
    }

    
    @Delete(':id')
    @HttpCode(204)
    // must have a valid token to delete
    @UseGuards(AuthGuardJwt)
    async remove(
        @Param('id', ParseIntPipe) id:number, 
        @CurrentUser() user :User) { 
